...
public class Put_tc2 {
	...
	public static void main(String[] args) {
		...
		// Configure baseURI
				...
				int statusCode=given().header("Content-type","application/json").body(req_body).when().put("api/users/2").then().extract().response().statusCode();
				System.out.println(statusCode);
				
				String res_body=given().header("Content-type","application/json").body(req_body).when().put("api/users/2").then().extract().response().asString();
